...
describe('parseResponse with plaintext (none) format', () => {
  ...
  // Regression: the parser unwraps a fenced block to find structured output, but for plain text
  // that truncated any response merely *containing* a code block. The default format prompts
  // explicitly tell the model to fence code, so this was easy to hit. Only a fence wrapping the
  // entire response is stripped now.
...
describe('parseResponse in schema mode', () => {
  // Regression: the <response> shortcut used to fire even with a schema, throwing away sibling
  // properties (`justification`) so structured revise responses always failed Zod validation.
  test('keeps sibling properties alongside <response>', () => {
    const schema = {
      type: 'object',
  ...
  // Regression: the XML revise prompt used to ask for a root-less fragment, which XMLValidator
  // rejects outright with "Multiple possible root nodes found".
  test('rejects a root-less multi-element fragment', () => {
  ...
  test('throws on unparseable JSON rather than salvaging', () => {
    const input = '{"response": invalid}';
...
describe('getPrefilled', () => {
  test('salvages a partially streamed JSON response on the way back in', () => {
    const partial = getPrefilled('The story so far', 'json');
    expect(parseResponse(partial, 'json')).toBe('The story so far');
  });
  ...
  test('produces a resumable, correctly escaped JSON prefill', () => {
    ...
    expect(JSON.parse(`${prefill}"\n}`).response).toBe(content);
  });
